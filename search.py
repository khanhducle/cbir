# USAGE
# python search.py --dataset ~/PyImageSearch/Datasets/ukbench/ukbench_sample --features-db output/features.hdf5 \
#	--bovw-db output/bovw.hdf5 --codebook output/vocab.cpickle \
#	--relevant ~/PyImageSearch/Datasets/ukbench/ukbench_sample/relevant.json \
#	--query ~/PyImageSearch/Datasets/ukbench/ukbench_sample/ukbench00760.jpg
from __future__ import print_function
from cbir.descriptors import DetectAndDescribe
from cbir.ir import BagOfVisualWords
from cbir.ir import Searcher
from cbir.ir import chi2_distance
...
nameIdx = args["query"].rfind("/")
name = args["query"][(nameIdx+1):]
if args["idf"] is None:
	cv2.imwrite("output/results/basic/query_{}".format(name), imutils.resize(queryImage, width=320))
else:
	cv2.imwrite("output/results/tfidf/query_{}".format(name), imutils.resize(queryImage, width=320))
...
if args["idf"] is None:
	cv2.imwrite("output/results/basic/results_{}".format(name), imutils.resize(montage.montage, height=700))
else:
	cv2.imwrite("output/results/tfidf/results_{}".format(name), imutils.resize(montage.montage, height=700))
